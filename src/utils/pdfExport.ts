...
export async function generateAuditPdf(
  ...
  doc.text('Best SAAS Metrics • Unit Economics Audit', margin + 6, 25);
  ...
  doc.text('Standard: SEC GAAP Gross Margin Allocation & SaaS Capital Efficiency Framework', margin + 6, 39);
  ...
  doc.setFontSize(8.5);
  doc.setFont('helvetica', 'bold');
  doc.text('AUDIT SIGN-OFF & METHODOLOGY GOVERNANCE', margin + 6, signY + 8);

  doc.setTextColor(71, 85, 105);
  doc.setFontSize(7.5);
  doc.setFont('helvetica', 'normal');
  doc.text('Lead Author: Alex Mercer (Former SaaS CFO & Venture Partner | 15+ Years SaaS Corporate Finance)', margin + 6, signY + 15);
  doc.text('Methodology Reviewer: Sarah Jenkins, CPA (Senior Director of Technical Accounting & SaaS Audit)', margin + 6, signY + 21);
  doc.text('Compliance: ASC 606 Sales Commission Capitalization & Bessemer Venture Partners (BVP) Cloud Index Standards', margin + 6, signY + 27);

  // Download PDF file
  const filename = `SaaS_CAC_Payback_Audit_${niche.slug}_${Date.now()}.pdf`;
  doc.save(filename);
}
