...
import { BookOpen, Calculator, DollarSign, ArrowUpRight, CheckCircle2, FileSpreadsheet } from 'lucide-react';
...
/**
 * High-Entity, Zero-Backlink Semantic SEO Architecture.
 * Targets high-intent queries (e.g. "how to calculate CAC payback", "ASC 606 customer acquisition cost formula", "SaaS unit economics math").
 * Uses semantic HTML5 `<section>`, `<dl>`, `<dfn>`, `<code>`, and `<math>` tags.
 */
    ...
    >
      ...
      <div className="space-y-2 border-b border-slate-200 pb-5">
        <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs font-bold shadow-2xs">
          ...
          <span>Institutional Corporate Finance Standard</span>
        </div>
        <h2
          id="formulas-math-title"
          className="text-xl sm:text-2xl font-black text-slate-900 tracking-tight"
        >
          SaaS Unit Economics Mathematical Formulation & GAAP Standards
        </h2>
        <p className="text-xs sm:text-sm text-slate-600 max-w-3xl leading-relaxed">
          The rigorous financial definitions and formulas governing Customer Acquisition Cost (CAC) Payback, Customer Lifetime Value (LTV), and Cash Breakeven under ASC 606 and venture capital audit frameworks.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-slate-900 text-white border border-slate-800 rounded-2xl p-6 space-y-4 shadow-md">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] font-mono font-bold bg-blue-500/20 text-cyan-300 border border-blue-400/30 px-2 py-0.5 rounded">
              Primary Standard
            </span>
          </div>
          ...
          <div className="p-3.5 bg-slate-950 border border-slate-800 rounded-xl font-mono text-xs text-slate-300 space-y-1.5 shadow-inner">
            ...
            <div className="text-xs font-bold text-slate-200 font-mono">
              ${inputs.cac.toLocaleString()} ÷ (${inputs.arpa.toLocaleString()} × {inputs.grossMargin}%) = <span className="text-cyan-400 font-black">{formatMonths(metrics.cacPaybackMonths)}</span>
            </div>
          </div>
        ...
        <div className="bg-slate-900 text-white border border-slate-800 rounded-2xl p-6 space-y-4 shadow-md">
          ...
          <div className="p-3.5 bg-slate-950 border border-slate-800 rounded-xl font-mono text-xs text-slate-300 space-y-1.5 shadow-inner">
            ...
            <div className="text-xs font-bold text-slate-200 font-mono">
              (${inputs.arpa.toLocaleString()} × {inputs.grossMargin}%) ÷ {inputs.churnRate}% = <span className="text-emerald-400 font-black">{formatCurrency(metrics.ltv)}</span>
            </div>
          </div>
      ...
      <div className="space-y-4 pt-4 border-t border-slate-200">
        ...
        <dl className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs text-slate-600">
          ...
          <div className="p-4 bg-slate-50/70 border border-slate-200 rounded-xl space-y-1.5">
            ...
            <dd className="leading-relaxed">
              Calculated as <code className="bg-slate-200 text-slate-800 px-1 py-0.5 rounded">ARPA × Gross Margin %</code>. Represents the exact monthly cash remaining after cloud infrastructure, customer success hosting, and third-party API costs are subtracted.
            </dd>
          </div>
